...
- A marker's `Position` restricts where it may open a comment. `COLUMN_ONE` matches at index 0 of the **untrimmed** line, which fixed-form Fortran needs: a column-one `C` opens a comment, the `C` of an indented `CALL FOO` does not. `LINE_START` matches the first non-blank character, which Vim Script needs for its leading `"`. Everything else is `ANYWHERE`.
- A `Quote.stringOrComment(...)` delimiter opens a literal when it closes on the line and a **comment** when it does not. This is how Vim Script's single overloaded `"` is resolved: `if exists("x")` is code, `set number  " why` is code plus comment.
...
- Vim Script's `"` is resolved by heuristic, not by parsing: leading `"` is a comment, and elsewhere an unclosed `"` is a comment. It misreads a comment that follows an unterminated single-quoted string, and a `"` after a `|` command separator. Measured against Neovim's runtime it lands within about 2% of an independent estimate.
- `.scm` is labelled Scheme, but in a Neovim tree those files are Tree-sitter queries. Both use `;` line comments, so the counts are right and only the name is misleading.
